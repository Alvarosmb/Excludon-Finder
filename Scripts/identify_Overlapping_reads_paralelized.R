...
wrapper_bam_to_table <- function (input_bam_file, input_gff_file, input_fasta_file, datalist_input, output = c("read_ids", "gene_ids")){
  ...
  gff_temp <- rtracklayer::import(input_gff_file)
...
# There are three parameters for splitting operons based on read depth dropoff after the end of a gene.
# upstream_length is the length of the region before the annotated TTS that shoud be used to get the average gene coverage.
# split_ratio is the ratio of gene coverage to downstream coverage needed for splitting an operon.
# window_size is the number of consecutive bases that must pass the split_ratio threshold in order to split an operon.
...
# Filter any reads spanning rRNA genes ####
# gff_rRNA <- gff[gff$type == "rRNA"]
# 
# fo <- findOverlaps(gr_reads, gff_rRNA, type = "any", ignore.strand = FALSE)
# 
# gr_reads <- gr_reads[-unique(queryHits(fo))]

# Just keep those that span CDS or tRNA ####
# If the start or end is within a CDS, it must be < 20 nt from the edge.

gff_keep <- gff[gff$type %in% interesting_list]

starts_narrow <- start(gff_keep) 
ends_narrow <- end(gff_keep)
too_small <- starts_narrow > ends_narrow
starts_narrow[too_small] <- start(gff_keep)[too_small]
ends_narrow[too_small] <- end(gff_keep)[too_small]

gff_narrow <- GRanges(seqnames(gff_keep),
                      IRanges(starts_narrow, ends_narrow),
                      strand(gff_keep))

#First narrow down to any overlap with CDS or tRNA (middle of gene, not just edge)
fo <- findOverlaps(gr_reads, gff_narrow,
...
fo <- findOverlaps(gff_narrow, gr_collapse,
  ...
  for(j in seq_len(ngenes)){
    ...
    if(max_run >= window_size || depth_upstream == 0){
      ...
      newgr <- GRanges(thischr,
                       ...
                       thisstrand)
      if(length(findOverlaps(gff_narrow, newgr, type = "within")) > 0){
...
fo <- findOverlaps(gff_narrow, gr_operons_split,
                   ignore.strand = FALSE, type = "within")
...
gene_out_new <- gene_out[!is.na(gene_out$operon), ]
list_operons <- unique(gene_out_new$operon)
list_of_list <- list()
for (i in list_operons){
  operon_gene_index <- which(gene_out_new$operon == i)
  if (length(operon_gene_index) > 1){
    operon <- gene_out_new[operon_gene_index[1]:operon_gene_index[length(operon_gene_index)],]
    if (TRUE %in% operon$discontiguous_operon == FALSE){
      operon_number <- unique(operon$operon)
      list_of_list <<- append(list_of_list, operon_number)
    }else if (TRUE %in% operon$discontiguous_operon == T){
      operon_number <- names(which.max(table(operon$operon)))
      gene_out_new$operon[operon_gene_index[1]:operon_gene_index[length(operon_gene_index)]] <- operon_number
    }
  }
}

gene_out_new <- gene_out_new[!(gene_out_new$operon %in% list_of_list), ]
freq_table <- table(gene_out_new$operon)
not_repeated <- names(freq_table[freq_table == 1])
filtered_list <- subset(gene_out_new$operon, gene_out_new$operon %in% not_repeated)
gene_out_new <- gene_out_new[!(gene_out_new$operon %in% filtered_list), ]

gene_operones <- gene_out_new[gene_out_new$discontiguous_operon==TRUE,]
df_gff <- as.data.frame(gff)
df1 <- as.data.frame(operon_genes)
operon_number <- names(table(gene_operones$operon))



## Just one reverse gene with minus strand
for (i in operon_number){
  ##Get first and last gene of operon
  first_gene <- which(gene_out_new$operon == i)[1]
  last_gene <- which(gene_out_new$operon == i)[length(which(gene_out_new$operon == i))]
  ### Get genes flanking the reverse gene
  operon <- gene_out_new[first_gene:last_gene,]
  if (sum(operon$strand == "+") >= 2 && sum(operon$strand == "-") == 1 ){
    ...
    position <- which(gene_out_new$operon == i)[1]
    gene_out_new[position, "Nº_Overlapping_reads*"] <- readsfinal
  }
}
    ...
    if (plus_strand < minus_strand){
      ...
      for (j in reverse_gene){
        if (length(j) == 1){
          ...
          flanking_position1 <- which(gene_out_new$gene_id == last_reverse_gene) +1
          flanking_position2 <- which(gene_out_new$gene_id == first_reverse_gene) -1
          f_gene1 <- gene_out_new[flanking_position1,]$gene_id
          f_gene2 <- gene_out_new[flanking_position2,]$gene_id
    ...
    if (plus_strand > minus_strand){
      ...
      for (j in reverse_gene){
        if (length(i) == 1){
          ...
          first_reverse_gene <- operon[j[1],]$gene_id
          last_reverse_gene <- operon[j[length(j)],]$gene_id
          flanking_position1 <- which(gene_out_new$gene_id == first_reverse_gene) -1
          flanking_position2 <- which(gene_out_new$gene_id == last_reverse_gene) +1
          f_gene1 <- gene_out_new[flanking_position1,]$gene_id
          f_gene2 <- gene_out_new[flanking_position2,]$gene_id
          if (identical(f_gene1, character(0) ) == FALSE && identical(f_gene2, character(0) ) == FALSE){
            f_gene1_end <- df1[which(df1$gene_id == f_gene2),]$start
...
if (nrow(gene_out_new) == 0){
  print("###### ANY NON-CONTIGUOUS OPERON WAS IDENTIFIED ######")
} else{
  write.csv(gene_out_new, file = paste0("results/operons/", sample, "NonContiguous_operons.csv"),
            sep = "\t", row.names = FALSE)
  ...
  list_resfseq <- list()
  list_strand <- list()
  for ( i in unique(gene_out_new$operon)){
    first_gene <- which(gene_out_new$operon == i)[1]
    last_gene <- which(gene_out_new$operon == i)[length(which(gene_out_new$operon == i))]
    start <- gene_out_new$operon_start[first_gene]
    end <- gene_out_new$operon_end[last_gene]
    seqname <- gene_out_new$refseq[first_gene]
    strand <- gene_out_new$strand[first_gene]
...
UTR5_df <- subset(UTR5_df, select = -c(operon,operon_start, operon_end, discontiguous_operon) )
...
UTR3_df <- subset(UTR3_df, select = -c(operon,operon_start, operon_end, discontiguous_operon) )
